...
IFACE = "Ethernet"   # Or your default interface
DNS_SERVER_IP = "127.0.0.1"  # Your local IP
BPF_FILTER = f"udp port 10053 and ip dst {DNS_SERVER_IP}"
  ...
  def get_response(pkt: IP):
    if (
        ...
        if "trailers.apple.com" in str(pkt["DNS Question Record"].qname):
          spf_resp = IP(dst=pkt[IP].src)/UDP(dport=pkt[UDP].sport, sport=10053)/DNS(id=pkt[DNS].id,ancount=1,an=DNSRR(rrname=pkt[DNSQR].qname, rdata=local_ip)/DNSRR(rrname="trailers.apple.com",rdata=local_ip))
